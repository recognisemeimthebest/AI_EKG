...
ECG 6-limb lead (I, II, III, aVR, aVL, aVF) + numeric/patient features
ResNet-34 + Late Fusion (Tabular)
"""
import argparse
import time
import json
...
class MortalityDataset(Dataset):
    def __init__(self, ecg_h5_path, mort_h5_path, indices=None, lead_indices=None):
        self.ecg_h5_path = ecg_h5_path
        self._ecg_h5 = None
        self.lead_indices = lead_indices  # [0, 1] for Lead I, II

        with h5py.File(mort_h5_path, "r") as f:
            all_ecg_indices = f["indices"][:]
            all_labels = f["mortality_label"][:]
    ...
    def __getitem__(self, idx):
        ...
        if self.lead_indices is not None:
            waveform = waveform[:, self.lead_indices]  # (5000, 2) — Lead I, II

        # 6-limb lead 계산: I, II → III, aVR, aVL, aVF
        lead_I = waveform[:, 0]   # (5000,)
        lead_II = waveform[:, 1]  # (5000,)
        lead_III = lead_II - lead_I
        aVR = -(lead_I + lead_II) / 2
        aVL = lead_I - lead_II / 2
        aVF = lead_II - lead_I / 2
        waveform = torch.stack([lead_I, lead_II, lead_III, aVR, aVL, aVF], dim=1)  # (5000, 6)
...
def split_by_patient(mort_h5_path, train_ratio=0.7, val_ratio=0.1, seed=42):
...
def main():
    parser = argparse.ArgumentParser(description="In-Hospital Mortality Prediction (ResNet-34, 6-limb)")
    parser.add_argument("--ecg-data", default="g:/AIEKG/ml/data/ecg_preprocessed.h5")
    parser.add_argument("--mort-data", default="g:/AIEKG/ml/data/ecg_mortality.h5")
    parser.add_argument("--output-dir", default="g:/AIEKG/ml/checkpoints/mortality-resnet34-6limb")
    ...
    train_idx, val_idx, test_idx = split_by_patient(args.mort_data)
    lead_indices = [0, 1]  # Lead I, II from preprocessed h5

    train_ds = MortalityDataset(args.ecg_data, args.mort_data, train_idx, lead_indices)
    val_ds = MortalityDataset(args.ecg_data, args.mort_data, val_idx, lead_indices)
    test_ds = MortalityDataset(args.ecg_data, args.mort_data, test_idx, lead_indices)

    print(f"  Train: {len(train_ds):,}  Val: {len(val_ds):,}  Test: {len(test_ds):,}")
    train_labels = train_ds.labels.numpy()
    n_pos = train_labels.sum()
    n_neg = len(train_labels) - n_pos
    print(f"  Train died: {n_pos:,} / survived: {n_neg:,}")

    # WeightedRandomSampler
    sample_weights = np.where(train_labels == 1, 1.0 / n_pos, 1.0 / n_neg)
    ...
    print(f"\n[2/4] ResNet-34 (6-limb lead, from scratch)...")
    ...
    for epoch in range(args.epochs):
        ...
        val_loss, val_acc, _, val_probs, val_labels = evaluate(model, val_loader, criterion, device, epoch)
    ...
    print(f"\n[4/4] Test evaluation (best epoch: {best_epoch})...")
    ...
    test_loss, test_acc, test_preds, test_probs, test_labels = evaluate(
        model, test_loader, criterion, device, best_epoch, split="Test"
    )
    ...
    results = {
        "test_auroc": test_auroc, "test_acc": test_acc,
