...
class HFDataset(Dataset):
    def __init__(self, ecg_h5_path, hf_h5_path, indices=None, lead_indices=None):
        self.ecg_h5_path = ecg_h5_path
        self._ecg_h5 = None
        self.lead_indices = lead_indices

        with h5py.File(hf_h5_path, "r") as f:
            all_ecg_indices = f["indices"][:]
            all_labels = f["hf_label"][:]
...
def split_by_patient(hf_h5_path, train_ratio=0.7, val_ratio=0.1, seed=42):
...
def main():
    parser = argparse.ArgumentParser(description="Heart Failure Detection (ResNet-34, 6-limb)")
    parser.add_argument("--ecg-data", default="g:/AIEKG/ml/data/ecg_preprocessed.h5")
    parser.add_argument("--hf-data", default="g:/AIEKG/ml/data/ecg_hf.h5")
    parser.add_argument("--output-dir", default="g:/AIEKG/ml/checkpoints/hf-resnet34-6limb")
    ...
    train_idx, val_idx, test_idx = split_by_patient(args.hf_data)
    lead_indices = [0, 1]

    train_ds = HFDataset(args.ecg_data, args.hf_data, train_idx, lead_indices)
    val_ds = HFDataset(args.ecg_data, args.hf_data, val_idx, lead_indices)
    test_ds = HFDataset(args.ecg_data, args.hf_data, test_idx, lead_indices)

    print(f"  Train: {len(train_ds):,}  Val: {len(val_ds):,}  Test: {len(test_ds):,}")
    train_labels = train_ds.labels.numpy()
    n_pos = train_labels.sum()
    n_neg = len(train_labels) - n_pos
    print(f"  Train HF: {n_pos:,} / non-HF: {n_neg:,}")
