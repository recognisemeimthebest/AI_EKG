...
변경 사항 (vs v1 AUROC 0.824):
1. 3-lead → 6-limb lead (I, II → III, aVR, aVL, aVF)
2. Class weight → WeightedRandomSampler + CE (검증된 조합)
3. Clinical features 유지 (DM, HF, MI, AHT)
"""
import argparse
import time
import json
...
class ParoxysmalAFDataset6Limb(Dataset):
    """6-limb lead + clinical features"""
...
def main():
    parser = argparse.ArgumentParser(description="Paroxysmal AF v2 (6-limb + Sampler)")
    ...
    parser.add_argument("--output-dir", default="g:/AIEKG/ml/checkpoints/paroxysmal-af-resnet34-v2")
    ...
    train_ds = ParoxysmalAFDataset6Limb(args.ecg_data, args.paf_data, train_idx, lead_indices)
    val_ds = ParoxysmalAFDataset6Limb(args.ecg_data, args.paf_data, val_idx, lead_indices)
    test_ds = ParoxysmalAFDataset6Limb(args.ecg_data, args.paf_data, test_idx, lead_indices)

    print(f"  Train: {len(train_ds):,}  Val: {len(val_ds):,}  Test: {len(test_ds):,}")
    train_labels = train_ds.labels.numpy()
    n_pos = train_labels.sum()
    n_neg = len(train_labels) - n_pos
    print(f"  Train PAF: {n_pos:,} / Normal: {n_neg:,} (ratio 1:{n_neg/n_pos:.1f})")

    # WeightedRandomSampler (검증된 조합: sampler + CE without class weight)
    ...
    print(f"\n[2/4] ResNet-34 (6-limb + clinical)...")
    n_numeric = train_ds.numeric.shape[1]  # 4 numeric + 4 clinical = 8
    n_patient = train_ds.patient.shape[1]  # 2
    ...
    print(f"  Numeric features: {n_numeric} (4 ECG + 4 clinical)")

    # CE without class weight (sampler handles balance)
    ...
    results = {
        ...
        "test_loss": float(test_loss), "best_epoch": best_epoch,
        "args": vars(args),
